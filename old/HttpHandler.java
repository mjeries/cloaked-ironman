...
public class HttpHandler implements Runnable {
	...
	private void processRequest() throws Exception {
		...
		// grab the file name, including the '/', and append a '.' in front of
		// it.
/*
		StringTokenizer tokens = new StringTokenizer(requestLine);
		tokens.nextToken(); // ignore GET token
		requestedFileName = tokens.nextToken();
		requestedFileName = "." + requestedFileName; // look within current
														// directory
*/
		/*
		 * figure out which file is being requested and (determine whether or
		 * not such a file exists) construct a properly formatted HTTP response
		 * message send the response
		 */
	...
	private static void sendBytes(FileInputStream fis, OutputStream os)
		...
		while ((bytes = fis.read(buf)) != -1) { // grab from file into the
												// buffer
