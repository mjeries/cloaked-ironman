...
public class Encryption {
	...
	public static String encrypt(final String plainText, final String symKeyHex) {
		...
		final byte[] encodedMessage = plainText.getBytes(Charset
				.forName("UTF-8"));
		...
		try {
			...
			final byte[] ivCipherText = new byte[ivData.length
					+ cipherText.length];
			System.arraycopy(ivData, 0, ivCipherText, 0, blockSize);
			System.arraycopy(cipherText, 0, ivCipherText, blockSize,
					cipherText.length);
			final String ivCipherTextBase64 = DatatypeConverter
					.printBase64Binary(ivCipherText);
	...
	public static String decrypt(final String ivCipherTextBase64,
			final String symKeyHex) {

		final byte[] symKeyData = DatatypeConverter.parseHexBinary(symKeyHex);
		final byte[] ivCipherText = DatatypeConverter
				.parseBase64Binary(ivCipherTextBase64);
		...
		try {
			...
			System.arraycopy(ivCipherText, blockSize, cipherText, 0,
					cipherText.length);
			...
			// concatenate IV and encrypted message
			final String plainText = new String(encodedMessage,
